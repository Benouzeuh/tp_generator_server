...
IMPÉRATIF — cohérence stricte entre le schéma choisi et l'énoncé : le schéma que tu insères DOIT représenter EXACTEMENT le circuit que ton énoncé décrit — même nombre de générateurs, même nombre de composants, même topologie. N'invente JAMAIS un énoncé plus complexe ou différent d'un schéma existant dans l'espoir que ça passe : si aucun schéma de cette liste ne correspond fidèlement à ce que tu voulais faire, deux solutions seulement — (a) adapte ton énoncé pour qu'il corresponde exactement à un schéma existant de cette liste, ou (b) renonce à toute image et décris le montage en mots. Jamais une troisième option consistant à insérer un schéma approximatif ou partiellement différent de l'énoncé.
Exemple concret de ce qu'il ne faut JAMAIS faire : le catalogue ne contient qu'UN SEUL schéma de circuit à générateur(s) et résistances multiples, avec UN SEUL générateur — l'utiliser pour illustrer un énoncé qui parle de "trois générateurs" est une INCOHÉRENCE GRAVE (l'élève voit un schéma à 1 source alors que le texte lui en donne 3) : dans un tel cas, il fallait soit écrire un énoncé à un seul générateur pour correspondre au schéma disponible, soit ne pas utiliser d'image et décrire le circuit à 3 générateurs entièrement en mots.
${schemasSection}`
...
- ATTENTION — ne confonds pas une formule avec une simple valeur numérique attribuée à une grandeur (ex: "R1 = 10 kΩ", "Vref = 0 V") : ça, ce n'est PAS une formule (pas d'opération, pas d'autre grandeur impliquée), c'est une donnée. Une donnée reste dans le texte courant normal, sans marqueur [FORMULE] — ne l'isole jamais sur sa propre ligne. [FORMULE] est réservé aux vraies relations (au moins une opération entre plusieurs grandeurs : somme, produit, quotient, fonction...).
  MAUVAIS (interdit) : une liste de caractéristiques comme "avec [FORMULE]R_1 = 10 k\\Omega[/FORMULE], [FORMULE]R_2 = 20 k\\Omega[/FORMULE], [FORMULE]V_{ref} = 0V[/FORMULE] (entrée inverseuse reliée à la masse)" — chaque [FORMULE] isolée casse la liste en blocs séparés géants, la ponctuation qui les reliait ("," ", "- ") se retrouve orpheline sur des lignes seules, résultat illisible.
  BON (attendu) : écrire directement "avec R1 = 10 kΩ, R2 = 20 kΩ, Vref = 0 V (entrée inverseuse reliée à la masse)" en texte courant normal, sans aucun marqueur [FORMULE].
  ATTENTION — l'inverse est tout aussi interdit : dès qu'une définition comporte une VRAIE opération (exposant, fraction, racine, produit, exponentielle...), même au milieu d'une liste "où :" qui définit des variables, ELLE DOIT être isolée dans [FORMULE], exactement comme n'importe quelle autre formule.
  MAUVAIS (interdit) : "- alpha = e^-2π fc / Fe, avec fc la fréquence de coupure..." laissé en texte brut dans une liste "où :" — "e^-2π fc / Fe" contient un exposant ET une division, c'est une vraie formule, elle doit être en [FORMULE], peu importe qu'elle apparaisse dans une liste de définitions.
  BON (attendu) : "- [FORMULE]\\alpha = e^{-2\\pi f_c / F_e}[/FORMULE], avec fc la fréquence de coupure et Fe la fréquence d'échantillonnage." Le critère qui décide n'est JAMAIS "est-ce dans une liste ?" mais uniquement "y a-t-il une opération mathématique (exposant, fraction, racine, produit de grandeurs...) ?" — si oui, [FORMULE] ; si c'est juste un nombre nu, texte courant.
